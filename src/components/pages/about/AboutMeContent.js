export const AboutMeInfo = () => {
    return (
        <div className="about_site_info">            
            Hello, I’m Justin, a third year computer science student at the 
            <a href="https://www.ualberta.ca/computing-science/index.html"> University of Alberta </a>  
            
            <br></br> <br></br>
            Outside of computers, I like to run and hike (when it isn't -30) and play hockey 
            and soccer. I enjoy reading, although if I'm in school my 
            book completion rate falls enormously. I also enjoy writting as a practice 
            for remembering and integrating otherwise scattered thoughts and ideas.  

        </div>
    );
}

export default AboutMeInfo;